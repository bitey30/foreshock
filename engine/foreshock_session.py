...
def log_packet(session, file, event, tier, api, flagged=()):
    """Append a packet record (rating starts null); return its session-local id.
    `flagged` = the dependent files foreshock surfaced (the → ones) — used for the behavioral proxy."""
    os.makedirs(SESS_DIR, exist_ok=True)
    rows = _read(session)
    pid = len(rows) + 1
    rec = {"id": pid, "ts": time.time(), "file": file, "event": event or "PostToolUse",
           "tier": tier or "", "api": bool(api), "flagged": list(flagged), "rating": None, "note": ""}
...
def summarize(session):
    ...
    if rated:
        ...
        out.append(f"  explicit rating: {avg:.1f}/5    [{dist}]   (model-judged, {len(rated)}/{len(rows)} covered)")
    ...
    if len(rows) - len(rated):
        out.append(f"  {len(rows) - len(rated)} packet(s) had no explicit rating (behavioral signal still covers them)")
    return "\n".join(out)
